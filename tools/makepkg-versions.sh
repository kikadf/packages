...
_WATCH_5=$(lynx -dump -listonly "https://github.com/meganz/MEGAcmd/tags" | sed -n 's:.*archive.\(.*[0-9]\)_Linux.*\.tar\.gz:\1:p' | head -1)
...
_WATCH_9=$(lynx -dump -nolist "https://codemadness.org/releases/sfeed/" | sed -n "s|sfeed-\(.*\).tar.gz .*|\1|p" | sort -V | tail -1)
