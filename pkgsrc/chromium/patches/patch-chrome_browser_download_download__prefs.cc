...
--- chrome/browser/download/download_prefs.cc.orig	2024-03-06 00:14:40.805965400 +0000
...
+#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_BSD)
   ...
   if (download_path == home_dir) {
     return true;
@@ -179,7 +184,7 @@ DownloadPrefs::DownloadPrefs(Profile* pr
...
@@ -472,7 +477,7 @@ void DownloadPrefs::DisableAutoOpenByUse
 }
...
+    BUILDFLAG(IS_MAC) || BUILDFLAG(IS_BSD)
 void DownloadPrefs::SetShouldOpenPdfInSystemReader(bool should_open) {
   if (should_open_pdf_in_system_reader_ == should_open)
     return;
@@ -504,7 +509,7 @@ bool DownloadPrefs::ShouldOpenPdfInSyste
...
+#elif BUILDFLAG(IS_WIN) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_BSD)
 ...
 #else
   return false;
@@ -668,7 +673,14 @@ base::FilePath DownloadPrefs::SanitizeDo
 #else
